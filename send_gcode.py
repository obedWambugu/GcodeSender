...
class GCodeSenderApp:
    def __init__(self, root):
        ...
        self.root.geometry("800x800")  # Slightly wider for jog controls
        ...
        # X Axis
        self.x_plus_button = tk.Button(jog_frame, text="0+", command=lambda: self.jog_axis("1", 1), width=5, state="disabled")
        self.x_plus_button.grid(row=1, column=0, padx=2, pady=2)
        self.x_minus_button = tk.Button(jog_frame, text="0-", command=lambda: self.jog_axis("1", -1), width=5, state="disabled")
        self.x_minus_button.grid(row=1, column=1, padx=2, pady=2)

        # Y Axis
        self.y_plus_button = tk.Button(jog_frame, text="D2+", command=lambda: self.jog_axis("2", 1), width=5, state="disabled")
        self.y_plus_button.grid(row=1, column=2, padx=2, pady=2)
        self.y_minus_button = tk.Button(jog_frame, text="D2-", command=lambda: self.jog_axis("2", -1), width=5, state="disabled")
        self.y_minus_button.grid(row=1, column=3, padx=2, pady=2)

        # Z Axis
        self.z_plus_button = tk.Button(jog_frame, text="D3+", command=lambda: self.jog_axis("3", 1), width=5, state="disabled")
        self.z_plus_button.grid(row=1, column=4, padx=2, pady=2)
        self.z_minus_button = tk.Button(jog_frame, text="D3-", command=lambda: self.jog_axis("3", -1), width=5, state="disabled")
        self.z_minus_button.grid(row=1, column=5, padx=2, pady=2)
    ...
    def jog_axis(self, axis, direction):
        ...
        distance *= direction  # Positive or negative
        command = f"J{axis} D{distance:.3f} F{feedrate}"
        self.log(f"Jogging {axis} {'+' if direction > 0 else '-'}{abs(distance)} mm at {feedrate} mm/min")
    ...
    def send_manual_command(self, command=None):
        ...
        try:
            ...
            prompt_seen = False
            while time.time() - start_time < 60:  # Match send_gcode_line timeout
    ...
    def connect_serial(self):
        ...
        try:
            ...
            self.x_plus_button.config(state="normal")
            self.x_minus_button.config(state="normal")
            self.y_plus_button.config(state="normal")
            self.y_minus_button.config(state="normal")
    ...
    def disconnect_serial(self):
            ...
            if self.serial and self.serial.is_open:
                ...
                self.x_plus_button.config(state="disabled")
                self.x_minus_button.config(state="disabled")
                self.y_plus_button.config(state="disabled")
                self.y_minus_button.config(state="disabled")
